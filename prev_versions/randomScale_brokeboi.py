...
class KeyOptions:
    ...
    def __init__(self, master):
        ...
        self.keyPopulate = EntityChecker(self.spot, keysAvailable, keysSelected, key = True)
        self.sharpFlatPopulate = EntityChecker(self.spot, sharpFlatAvailable, sharpFlatSelected, key = True)
    ...
    def refresh(self):
        ...
        for i in self.keyPopulate.returnedList:
            keysSelected += [i]
        for i in self.sharpFlatPopulate.returnedList:
            sharpFlatSelected += [i]
    ...
    def pack(self, **options):
        ...
        self.sharpFlatPopulate.pack(anchor = tk.W, key = True)
...
class ScaleOptions:
    ...
    def __init__(self, master):
        ...
        self.scalePopulate = EntityChecker(self.spot, scalesAvailable, scalesSelected)

    def refresh(self):

        scalesSelected = []

        for i in self.scalePopulate.returnedList:
            scalesSelected += [i]
...
class ChordOptions:
    ...
    def __init__(self, master):
        ...
        #self.box = tk.Listbox(self.spot, bd = 0)
        self.chordPopulate = EntityChecker(self.spot, chordsAvailable, chordsSelected)

    def refresh(self):

        chordsSelected = []

        for i in self.chordPopulate.returnedList:
            chordsSelected += [i]
...
class EntityChecker:
    ...
    """
    def __init__(self, master, availableList, selectedList, **options):
        ...
        if (keyOption == True):
            self.location = tk.Frame(master,bd = 0.5, relief = tk.RIDGE)
            self.box = tk.Listbox(self.location, selectmode = "multiple", bd = 0, height = len(availableList), exportselection = False)
            self.box.config()
        else:
            self.location = tk.Frame(master, borderwidth = 2,relief = tk.RIDGE)
    ...
    def update(self):
        ...
        for i in self.availableList:
            print("<PARSING : " + str(i) + ">")
            if (i == "natural") and (self.availableList.index(i) in self.box.curselection()):
                self.returnedList.remove("")
            elif (self.availableList.index(i) in self.box.curselection()):
